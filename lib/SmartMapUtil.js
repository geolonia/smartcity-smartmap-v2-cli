...
class SmartMapUtil {
    ...
    constructor(initConfig) {
        ...
        this.MBTilePath = initConfig.MBTilePath;
    }

    async build() {
        await this.convertToGeoJSON();
        await this.createVectorTiles();
        // await this.createMenuYAML();
    }
        ...
        for (let i = 0; i < this.config.length; i++) {
            ...
            if (data['データ種別'] === 'geojson') {
                ...
                if (data['データ参照先'].startsWith('http')) {
                    const dataPath = path.join(this.dataDir, `${data['レイヤー名']}.geojson`);
             ...
             */
            if (data['データ種別'] === 'shape') {
                const dataPath = path.join(this.dataDir, `${data['データ参照先']}.geojson`);
                const shapePath = path.join(this.dataDir, `${data['データ参照先']}.shp`);
    ...
    async createVectorTiles() {
        await geojsonToMBTile(this.dataDir, this.MBTilePath);
    }
