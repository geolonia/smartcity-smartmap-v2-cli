...
const configToMenuYAML = (config, outputFile) => {
  ...
  config.forEach(item => {
    const { 大カテゴリー, 中カテゴリー, メニュータイトル, タイルレイヤー名, データ参照先, データ種別, geojsonPath } = item;
    ...
    if (メニュータイトル) {
      ...
      if (大カテゴリー && 中カテゴリー && メニュータイトル) {
        menu[大カテゴリー].items[中カテゴリー].items[メニュータイトル] = { type: 'data', tileId: タイルレイヤー名 };
        return;
      }

      // 大カテゴリとメニュータイトルがある場合
      if (大カテゴリー && メニュータイトル) {
        menu[大カテゴリー].items[メニュータイトル] = { type: 'data', tileId: タイルレイヤー名 };
        return;
      }

      // メニュータイトルのみの場合
      if (メニュータイトル) {
        menu[メニュータイトル] = { type: 'data', tileId: タイルレイヤー名 };
        return;
      }
    } else {
